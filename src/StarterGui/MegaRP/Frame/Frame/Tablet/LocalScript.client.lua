	...
	if button:IsA("TextButton") then
		button.MouseButton1Click:Connect(function()
			...
			if script.Parent.Screens:FindFirstChild(button.Name) then
				if button.Name == "Obywatele" then
					local screnframe = script.Parent.Screens:FindFirstChild(button.Name)
					
					for i, frame in screnframe.ActiveUsers.ScrollingFrame:GetChildren() do
					...
					for i, player in game.Players:GetChildren() do
						local activeuserframe = screnframe.ActiveUsers.ScrollingFrame.UIListLayout.Frame:Clone()
						
						warn(player)

						activeuserframe.ID.Text = player.UserId
						activeuserframe.UserName.Text = player:GetAttribute("Name") .. " " .. player:GetAttribute("LastName")
						activeuserframe.Parent = screnframe.ActiveUsers.ScrollingFrame
				...
				if button.Name == "EkranGlowny" then
					...
					for i, player in game.Players:GetChildren() do
						...
						warn(player)
						local activefaction = player:GetAttribute("FactionDuty")
						
						warn(activefaction)
						
						if activefaction then
							
							activeuserframe.Faction.Text = activefaction
							activeuserframe.UserName.Text = player:GetAttribute("Name") .. " " .. player:GetAttribute("LastName")
							activeuserframe.Parent = screnframe.ActiveUsers.ScrollingFrame
...
local screnframe = script.Parent.Screens.EkranGlowny
