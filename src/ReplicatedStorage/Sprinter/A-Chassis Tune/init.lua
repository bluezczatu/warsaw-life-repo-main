...
--[[Weight and CG]]
	Tune.Weight			= 5512		-- Total weight (in pounds, under Earth's gravity)
...
Tune.FSusDamping	= 900		-- Spring Damping
Tune.FSusLength		= 3.28			-- Base spring height (in studs)
...
Tune.RSusDamping	= 900		-- Spring Damping
Tune.RSusLength		= 3.25			-- Base spring height (in studs)
...
Tune.SteerDecay		= 290		-- Speed of gradient cutoff (in SPS)
...
--[[Engine (Avxnturador)]]
	...
	Tune.Engine 		= true		
		...
		Tune.Horsepower		= 120	-- Naturally aspirated horsepower, before adding forced induction
		Tune.IdleRPM		= 150	-- Engine idle
		Tune.PeakRPM		= 7750	-- Where horsepower reaches peak
		Tune.Redline		= 7800	-- Your engine's rev limiter
		Tune.PeakSharpness	= 11.5	-- How fast power drops off past peak horsepower
		Tune.CurveMult		= .7	-- Higher values mean a peakier power curve, lower values being smoother
	...
	Tune.Turbochargers	= 0				-- Number of turbochargers in the engine, set to 0 for no turbochargers
		...
		Tune.T_Boost		= 30
		Tune.T_BoostLag		= 35		-- Turbo lag overall
		Tune.T2_BoostLag	= 68		-- (ONLY ACTIVE ON TWIN TURBO CARS) Second turbo lag, equal to the first turbo lag or greater if you're using sequential turbos
	...
	--Misc
	Tune.ThrotAccel		= .2		-- Throttle acceleration, applied per tick (recommended values between 0.05 to 0.1)
...
--[[Drivetrain]]	
	...
	Tune.TorqueVector	= .63		-- AWD ONLY, "-1" has a 100% front bias, "0" has a 50:50 bias, and "1" has a 100% rear bias. Can be any number between that range.
...
--[[Transmission]]
	...
	--Automatic: DCT
	Tune.ShiftUpTime	= 0.205		-- Time required to shift into next gear, from a lower gear to a higher one.
	Tune.ShiftDnTime	= 0.1		-- Time required to shift into next gear, from a higher gear to a lower one.
	
	--Gear Ratios

Tune.FinalDrive		= 9.90 	 --		[TRANSMISSION CALCULATIONS FOR NERDS]
Tune.Ratios			= {			 --		SPEED [SPS]	 = (Wheel diameter(studs) * math.pi * RPM) / (60 * Gear Ratio * Final Drive * Multiplier)
	--[[ R ]]		4.90		,--		WHEEL TORQUE = Engine Torque * Gear Ratio * Final Drive * Multiplier
	--[[ N ]]		0			,
	--[[ 1 ]]		2.97		,
	--[[ 2 ]]		1.77		,
	--[[ 3 ]]		1.30		,
...
--[[Brakes]]
	...
	Tune.BrakeForce		= 7000		-- Total brake force (LuaInt)
